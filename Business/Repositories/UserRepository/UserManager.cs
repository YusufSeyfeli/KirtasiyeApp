...
namespace Business.Repositories.UserRepository
{
    public class UserManager : IUserService
    {
        ...
        {
            return new SuccessDataResult<User>(await _userDal.Get(p => p.InsanId == id));
        }

        public async Task<User> GetByIdForAuth(int id)
        {
            return await _userDal.Get(p => p.InsanId == id);
        }
        ...
        {
            var user = await _userDal.Get(p => p.InsanId == userChangePasswordDto.InsanId);
