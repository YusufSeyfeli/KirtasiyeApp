...
namespace Business.Repositories.OperationClaimRepository
{
    public class OperationClaimManager : IOperationClaimService
    {
        ...
        {
            IResult result = BusinessRules.Run(await IsNameExistForAdd(operationClaim.OperationClaimName));
            if (result != null)
            {
                return result;
            }
        ...
        {
            var result = await _operationClaimDal.Get(p => p.OperationClaimId == id);
        ...
        {
            var currentOperationClaim = await _operationClaimDal.Get(p => p.OperationClaimId == operationClaim.OperationClaimId);
