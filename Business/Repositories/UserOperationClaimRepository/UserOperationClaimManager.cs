...
namespace Business.Repositories.UserOperationClaimRepository
{
    public class UserOperationClaimManager : IUserOperationClaimService
    {
        ...
        public async Task<IResult> Delete(int userOperationClaim)
        {
        ...
        {
            return new SuccessDataResult<UserOperationClaim>(await _userOperationClaimDal.Get(p => p.KullaniciId == id));
        }
        ...
        public async Task<IResult> Update(UserOperationClaim userOperationClaim)
        {
            IResult result = BusinessRules.Run(
                await IsUserExist(userOperationClaim.KullaniciId),
        ...
        {
            var result = await _userOperationClaimDal.Get(p => p.KullaniciId == userOperationClaim.KullaniciId&& p.OperationClaimId == userOperationClaim.OperationClaimId);
            if (result != null)
            {
        ...
        {
            var currentUserOperationClaim = await _userOperationClaimDal.Get(p => p.KullaniciId == userOperationClaim.KullaniciId);
            if (currentUserOperationClaim.KullaniciId != userOperationClaim.KullaniciId || currentUserOperationClaim.OperationClaimId != userOperationClaim.OperationClaimId)
            {
                var result = await _userOperationClaimDal.Get(p => p.KullaniciId == userOperationClaim.KullaniciId && p.OperationClaimId == userOperationClaim.OperationClaimId);
                if (result != null)
                {
